 ...
 * Fecha: 12/08/2018
 *
 * Ejercicio 5: Semaforos y Contadores
 *
 * Ahora repitamos el ejercicio anterior para que tres tareas distintas
 * destellen, cada una, un led distinto de la EDU-CIAA, tras completar la
 * lectura la primera tarea.
 *
 */
...
void showLED2( void* taskParmPtr );
void showLED3( void* taskParmPtr );
...
int main(void)
{
   ...
   semaforo = xSemaphoreCreateCounting(3,0);
   ...
   // Creacion de la tarea showTime
   xTaskCreate(
	  showLED2,                     // Funcion de la tarea a ejecutar
      (const char *)"showLED2",     // Nombre de la tarea como String amigable para el usuario
      configMINIMAL_STACK_SIZE*2, 	  // Cantidad de stack de la tarea
      0,                              // Parametros de tarea
    ...
    // Creacion de la tarea showTime
    xTaskCreate(
      showLED3,                     // Funcion de la tarea a ejecutar
      (const char *)"showLED3",     // Nombre de la tarea como String amigable para el usuario
      configMINIMAL_STACK_SIZE*2, 	  // Cantidad de stack de la tarea
      0,                              // Parametros de tarea
...
void showLED1( void* taskParmPtr )
{
   ...
   // ---------- REPETIR POR SIEMPRE --------------------------
   while(TRUE) {
	   if(xSemaphoreTake( semaforo , 10000) == pdTRUE){
    	  gpioWrite( LEDB, ON );
